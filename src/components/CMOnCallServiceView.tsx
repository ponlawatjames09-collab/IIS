...
import { fetchCMOnCallRecords, appendCMOnCallRecord, updateCMOnCallRecord } from '../sheetsService';
...
interface CMOnCallServiceViewProps {
...
export default function CMOnCallServiceView({ settings, accessToken, onNotifyLine, onOpenCMJob }: CMOnCallServiceViewProps) {
  const targetSpreadsheetId = settings.cmOnCallSpreadsheetId || '19c9vJhO4qMdCP1O7rwWP_aUZyTqQrTwnzWWxtH7zDdE';
  const [records, setRecords] = useState<CMStatusRecord[]>([]);
  // CM On Call uses similar statuses
  ...
  const loadRecords = async () => {
    ...
    setIsLoading(true);
    setError(null);
    console.log('Loading records for CM On Call, spreadsheet ID:', targetSpreadsheetId);
    try {
      const data = await fetchCMOnCallRecords(targetSpreadsheetId, accessToken);
      console.log('CM On Call fetched data:', data);
      setRecords(data);
    } catch (err: any) {
      if (err.message && err.message.includes("403")) { setError("Failed to load CM On Call records. 403 Forbidden. Please make sure the Google Sheets API is enabled for this project and you have access to the spreadsheet."); } else { if (err.message && err.message.includes('401')) { setError('Session expired. Please sign out and sign in again.'); } else { setError(err.message || 'Failed to load CM On Call records.'); } }
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    loadRecords();
  }, [targetSpreadsheetId, accessToken]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editRowIndex !== null) {
        await updateCMOnCallRecord(targetSpreadsheetId, accessToken, editRowIndex, newRecord);
      } else {
        await appendCMOnCallRecord(targetSpreadsheetId, accessToken, newRecord);
      }
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 relative">
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between shrink-0">
        <div>
          <h1 className="font-bold text-slate-800">CM On Call (งานแจ้งซ่อม Warranty)</h1>
